...
Reads CIFAR-10 from cached HF parquet, converts to tensors in batch."""
...
def main():
    ...
    log(f"Reading from: {parquet_path}")
    table = pq.read_table(parquet_path)
    log(f"Rows: {table.num_rows}")
    ...
    for i in range(n):
        ...
        labels.append(label)
        if (i+1) % 2000 == 0:
            log(f"  Decoded {i+1}/{n} images ({time.time()-t0:.0f}s)")
    ...
    log(f"Running PGD-20 (eps={epsilon:.4f})...")
    t0 = time.time()
