...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\columnwidth]{assets/foveal_stream_arch.png}
\caption{VisualTorch architecture of the Foveal Stream ConvNet. The block processes a localized $48\times 48$ crop centered at $a_t$ and outputs a $512$-dimensional foveal feature vector representing local high-resolution details.}
\label{fig:foveal_stream}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\columnwidth]{assets/gaze_trajectory.png}
\caption{Matplotlib visualization of the Epistemic Gaze Foraging path. Starting from the center ($t=0$), the gaze moves dynamically toward the region with the highest local prediction error to gather foveal information.}
\label{fig:gaze_trajectory}
\end{figure}

Crucially, the gradient $\nabla_a E(a)$ backpropagated through a deep convolutional network (like the foveal stream) is naturally tiny in magnitude ($\approx 0.03$). Without normalizing this gradient to unit norm, the fovea would only shift by a fraction of a percent of the image space, resulting in static foveation. Normalizing by the L2 norm $\parallel \nabla_a E(a) \parallel_2$ guarantees that foveal attention shifts by exactly the intended step size $\eta(\Pi_D^{(t)})$, making the gaze trajectories active and diverse.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\columnwidth]{assets/precision_init_net_arch.png}
\caption{VisualTorch visualization of the context-dependent Precision Initialization Network. It maps the current global belief state $s_t$ to an initial sensory precision estimate in $[0.2, 0.8]$.}
\label{fig:precision_init}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\columnwidth]{assets/precision_vs_epoch.png}
\caption{Divergent class-wise convergence profiles of the sensory precision $\Pi_D$ across training epochs. Out-of-distribution or highly ambiguous classes (like car/truck under perturbation) converge to higher precision bounds near $0.80$, while easy classes converge toward $0.20$.}
\label{fig:precision_vs_epoch}
\end{figure}
...
\section{Hugging Face Cloud Synchronization and LFS Pruning}
Training deep active inference architectures over multiple epochs requires uploading checkpoints to the cloud to prevent local data loss in transient runtimes. Let $\Theta_k$ denote the serialized state dictionary at epoch $k$:
\begin{equation}
\Theta_k = \{k, W_k, \phi_k, \gamma_k, \delta_{\text{best}}\}
\end{equation}
where $W_k$, $\phi_k$, and $\gamma_k$ represent the model parameters, optimizer, and scheduler states respectively, and $\delta_{\text{best}}$ is the running validation accuracy.

Under standard Git LFS, committing a new checkpoint $\Theta_k$ at every epoch appends a new file version to the repository history. For $E = 60$ epochs and a checkpoint size of $|\Theta| \approx 250$ MB (which expands to $\approx 467$ MB when serializing active gradients and optimizer variables), the total LFS storage scales as:
\begin{equation}
\text{Storage Complexity} = O(E \cdot |\Theta|) \approx 15 \text{ GB} - 28 \text{ GB}
\end{equation}
which exceeds the global private dataset storage quota ($10$ GB) of the Hugging Face free tier.

To bypass this ceiling and achieve $O(1)$ LFS storage overhead, we isolate the rolling checkpoint to a dedicated public repository $R_{\text{rolling}}$ and execute a clean state reset on each push. The synchronization thread executes:
\begin{enumerate}
    \item \textbf{Purge}: Call $\mathcal{D}(R_{\text{rolling}})$ to delete the remote repository.
    \item \textbf{Re-creation}: Call $\mathcal{C}(R_{\text{rolling}})$ with public visibility after a short delay ($\Delta t_{\text{wait}} = 2$ s) to allow backend deletion to finish.
    \item \textbf{Upload}: Call $\mathcal{U}(R_{\text{rolling}}, \Theta_k)$ to push the new checkpoint.
\end{enumerate}
This forces the Git commit history of $R_{\text{rolling}}$ to remain at depth $1$ containing exactly one LFS object, keeping total LFS storage at exactly $1 \times 250$ MB. The best validation checkpoints are saved to a separate repository $R_{\text{best}}$ without deletion, which scales as $O(1)$ because validation accuracy improvements occur rarely.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\columnwidth]{assets/robustness_curve.png}
\caption{Robust test accuracy comparison under $L_\infty$ adversarial epsilon scaling. Under high-epsilon bounds ($\epsilon \ge 0.031$), static TRADES models suffer from a complete collapse in robustness, whereas RHAN-v10's active inference and dynamic precision maintain high robust accuracy.}
