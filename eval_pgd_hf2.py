...
Uses HuggingFace datasets from default cache location."""
import os, sys, time, torch, torch.nn as nn, torch.nn.functional as F

# Force offline mode BEFORE importing datasets
...
def main():
    ...
    # Load CIFAR-10 via HF datasets from cache
    from datasets import load_dataset
    import torchvision.transforms as transforms
    
    print("Loading CIFAR-10 test set from cache...", flush=True)
    ...
    lbls_list = []
    for item in test_ds:
        img = transform(item['img'].convert('RGB'))
        imgs_list.append(img)
        lbls_list.append(item['label'])
