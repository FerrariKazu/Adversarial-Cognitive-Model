#!/usr/bin/env python3
"""
Kaggle Notebook — RHAN-NX Generation 0 (Stage 5) Execution
=========================================================
...
──────────────────────────────────────────────────────────────────────────────
Kaggle had no RHAN-NX block before 2026-09-11 (it ended at Stage 4); this
notebook now mirrors the Colab Stage-5 runner. Stage 1-4 execution blocks
remain in git history (commits up to 8abb343 on feature/rhan-next).
...
if DO_RHAN_NX:
    ...
    def _nx_trainer(ckpt_name, max_epochs, extra, base, tag):
        ...
        # 2026-09-12: use ALL visible GPUs via torch.distributed DDP when >1
        # GPU is present (Kaggle T4x2). The trainer's DDP path shards the
        # sampler across ranks and keeps per-rank --accum-steps, so effective
        # batch is --batch-size x world_size. Micro-batch is HALVED here so
        # the effective batch and optimizer trajectory are IDENTICAL to the
        # single-GPU 16x16=256 recipe: 2 GPUs -> 8 per rank x 32 accum x 2
        ...
        #
        # DDP failures are SELF-HEALING (2026-09-12 Kaggle T4x2 incident:
        # bare `torchrun --nproc_per_node=2` died rc=1 — container
        # rendezvous/NCCL issues, also documented in kaggle_train_synthetic.py
        # "torchrun DDP has NCCL init issues in Kaggle containers"). A failed
        # DDP attempt falls back to the proven single-GPU recipe (identical
        # effective batch); the fingerprint-safe resume keeps the rolling
        # checkpoint, so the ladder never dies mid-stage.
        _micro, _accum = 16, 16
        _ngpu_note = ""
        _try_ddp = (not os.environ.get("RHAN_NX_SINGLE_GPU")
                    and int(torch.cuda.device_count() or 0) > 1)
        if _try_ddp:
            _ngpu = int(torch.cuda.device_count())
            _micro = max(16 // _ngpu, 1)
            _accum = 16 * 16 // (_micro * _ngpu)
            _ngpu_note = (f"  # DDP: {_ngpu} GPUs x {_micro} micro-batch "
                          f"x {_accum} accum = {_micro * _accum * _ngpu} "
                          f"effective (fallback: 16x16 single-GPU)")
            # NCCL hardening for containerized GPU pairs: no P2P over PCIe,
            # no InfiniBand, and a 15-min watchdog (first collective after
            # CUDA init on T4 pairs can be slow; a stuck run is killed and
            # the fallback takes over rather than hanging the session).
            os.environ.setdefault("NCCL_P2P_DISABLE", "1")
            os.environ.setdefault("NCCL_IB_DISABLE", "1")
            os.environ.setdefault("NCCL_TIMEOUT", "900")
        ...
        _cmd = (f"{'torchrun' if _try_ddp else 'python3'} "
        ...
        def _launch(cmd, base_args):
            """Print and run the trainer; DDP failures self-heal.

            The real DDP command uses `python3 -m torch.distributed.run
            --standalone --master-port=<free>`: no PATH dependency on the
            `torchrun` binary and no fixed-port c10d rendezvous contention
            inside containers (both killed the 2026-09-12 Kaggle attempt).
            On a DDP failure it falls back to the proven single-GPU recipe —
            same effective batch — and the trainer's resume guard continues
            from the rolling checkpoint (fingerprints match: same commit).
            """
            if DRY_RUN:
                print(f"  [{tag}] {cmd}{_ngpu_note}", flush=True)
                return
            if not _try_ddp:
            ...
            _ddp_cmd = (
                ...
                f"{base_args} "
                f"--batch-size {_micro} --accum-steps {_accum}")
            print(f"  [{tag}] {_ddp_cmd}{_ngpu_note}", flush=True)
            try:
                run(_ddp_cmd)
            except subprocess.CalledProcessError as e:
                print(f"  [DDP FAILED rc={e.returncode}] — falling back to "
                      f"the proven single-GPU recipe (identical effective "
                      f"batch 256); the resume guard keeps the rolling "
                      f"checkpoint.", flush=True)
                run(f"python3 phase1_training/train_rhan_next.py "
                    f"{base_args} --batch-size 16 --accum-steps 16")

        _launch(_cmd, _base_args)
