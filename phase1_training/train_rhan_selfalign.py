...
def generate_pgd_adversarial(model, x, labels, epsilon, alpha, steps,
    ...
    """PGD attack targeting the TRADES loss."""
    x = x.detach()
    ...
    with torch.no_grad():
        with autocast('cuda'):
            clean_logits, _ = model.forward_with_features(x)

    for _ in range(steps):
        x_adv.requires_grad_(True)
        with torch.enable_grad():
            with autocast('cuda'):
                adv_logits, _ = model.forward_with_features(x_adv)
                loss = F.kl_div(
...
def main():
    ...
    # ── TRAINING LOOP ──────────────────────────────────────────────────────
    print("=" * 70)
    print("SELF-ALIGNMENT FINE-TUNING (RHAN-v5)")
    print(f"Epochs: {epochs}, LR: 0.002, Beta: {beta}, Epsilon: {epsilon}, Steps: {steps}")
    print("=" * 70)

    best_acc = 0.0
    best_ckpt_path = os.path.join(ckpt_dir, 'rhan_selfalign_best.pth')
    rolling_ckpt_path = os.path.join(ckpt_dir, 'rhan_selfalign_rolling.pth')
    ...
    for epoch in range(1, epochs + 1):
        ...
        l_trades_sum = l_self_sum = l_freq_sum = 0
        ...
        for imgs, lbls in trainloader:
            ...
            with autocast('cuda'):
                ...
                # 4. Self-alignment loss: pull adversarial toward clean
                # CRITICAL: detach clean features so they are never modified
                feat_c_anchor = feat_c.detach()
                l_self = 1.0 - F.cosine_similarity(
                    F.normalize(feat_a, dim=-1),
                    F.normalize(feat_c_anchor, dim=-1)
                ).mean()
                ...
                # 6. Combined Loss
                loss = 0.55 * l_trades + 0.35 * l_self + 0.10 * l_freq
        ...
        print(
            ...
            f"TRADES: {l_trades_sum/n_total:.4f} | "
            f"l_self: {l_self_sum/n_total:.4f} | "
    ...
    print("python3 phase2_attacks/eval_autoattack.py --model checkpoints/rhan_selfalign_best.pth")
