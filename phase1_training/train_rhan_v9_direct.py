...
def generate_pgd_adversarial(model, x, labels, epsilon, alpha, steps,
    ...
    # We need the clean logits for TRADES kl_div calculation during attack
    with torch.no_grad():
        with autocast('cuda'):
...
def main():
    ...
    # ── TRAINING LOOP ──────────────────────────────────────────────────────
    print("=" * 70)
    print("DIRECT TRADES FINE-TUNING (RHAN-v9)")
    print(f"Epochs: {epochs}, LR: 0.0002, Beta: {beta}, Epsilon: {epsilon}, Steps: {steps}")
    print("=" * 70)

    best_acc = 0.0
    best_ckpt_path = os.path.join(ckpt_dir, 'rhan_v9_best.pth')
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch in [1, 5, 10, 20, 30]:
            ...
            # Print silently without cluttering the screen too much, or print normally since user asked to print every epoch
            print(
                f"Epoch {epoch:02d}/{epochs} | "
                f"Loss: {total_loss/n_total:.4f} | "
                f"TRADES: {l_trades_sum/n_total:.4f} | "
    ...
    print("python3 phase2_attacks/eval_autoattack.py --model checkpoints/rhan_v9_best.pth")
