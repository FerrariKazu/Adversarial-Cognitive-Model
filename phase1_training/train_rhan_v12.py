#!/usr/bin/env python3
"""
Experiment: RHAN-v12 Locked-In Active Inference Training on STL-10
===================================================================

Consolidates the isolation-run lessons into the final locked-in config:

  1. LOSS PIPELINE — exactly two terms:
         L = w_trades * L_trades + w_recon * L_recon
     The foraging-consistency, precision-calibration and halt-efficiency
     auxiliary losses are DELETED (not zero-weighted). Dynamic beta
     modulation beta_dyn = beta * (0.5 + Pi_D) is retained (Pi_D comes
     from the model's ImageSpacePrecision forward pass).

  2. FORAGING — fixed T = max_foraging_steps (default 4), no halt network.

  3. GAZE — reconstruction-guided: model.gaze_lambda blends the
     pixel-space reconstruction gradient with the feature-space
     gradient (--gaze-lambda, default 0.5).

Flags added vs v11:
    --gaze-lambda 0.5   weight of reconstruction-guided gaze (Eq. II v12)
    --no-pseudo         skip pseudo-label generation (real + synthetic only)
    --max-epochs 60     stop epoch (10 for diagnostics)

Saves to: checkpoints/<ckpt-name>_best.pth / _rolling.pth
"""
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# DATA PREPARATION & BALANCED LOADING (unchanged from v11)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# PSEUDO-LABEL GENERATION (unchanged from v11)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# DYNAMIC TRADES LOSS (v12: L = w_trades * L_trades + w_recon * L_recon only)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

def dynamic_trades_loss(model, imgs, labels, weights,
                        x_adv, beta_base):
    """
    TRADES loss with precision-weighted beta + reconstruction loss for the
    generative prior. This is the ONLY loss path in v12:
        l_total = (l_ce + beta_dynamic * l_kl) * weights        -> L_trades
        l_recon = mean reconstruction MSE (clean + adversarial) -> L_recon
    """
    # Forward pass on clean (with trajectory to get Pi_D)
    ...
    beta_dynamic = beta_base * (0.5 + final_precision_c)  # range: [beta/2, beta*1.5]
    ...
    # Dynamic weighting (real = 1.0, pseudo/synthetic = 0.5)
    l_trades = (l_ce + beta_dynamic * l_kl) * weights.to(l_ce.device)
    l_trades = l_trades.mean()
    ...
    return l_trades, traj_c, traj_a, beta_dynamic.detach(), l_recon


# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# DIAGNOSTIC TELEMETRY (v12: no foraging/precision-calibration diagnostics)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
class EpochDiagnostics:
    ...
    def update(self, beta_dyn, traj_c, labels):
        ...
        if 'recon_errors' in traj_c and len(traj_c['recon_errors']) > 0:
            self.recon_losses.append(
                traj_c['recon_errors'][-1].detach().cpu())
    ...
    def report(self, epoch, eps):
        ...
        print(f"\n{'─'*60}")
        print(f"  RHAN-v12 Diagnostics — Epoch {epoch} (ε={eps:.3f})")
        print(f"{'─'*60}")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# HUGGING FACE HELPERS (unchanged from v11)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def ensure_checkpoint_exists(ckpt_path):
    ...
    try:
        ...
        if 'v11' in filename or 'v12' in filename:
...
def main():
    parser = argparse.ArgumentParser(description='RHAN-v12 Locked-In Active Inference Training')
    ...
    parser.add_argument('--max-foraging-steps', type=int, default=4,
                        help='Fixed foraging steps (no halt network; loop always runs this many)')
    ...
    parser.add_argument('--metabolic-cost', type=float, default=0.05,
                        help='Metabolic cost threshold (retained for checkpoint compat)')
    # Loss weights — v12 has EXACTLY two loss terms
    ...
    # Eq. II v12 reconstruction-guided gaze
    parser.add_argument('--gaze-lambda', type=float, default=0.5,
                        help='Weight of reconstruction-guided gaze: lambda * grad R + (1-lambda) * grad F')
    parser.add_argument('--synthetic-data', type=str, default='',
                        help='Path to .pt file with synthetic data (dict with imgs, labels)')
    parser.add_argument('--ckpt-name', type=str, default='rhan_stl10_v12',
                        help='Checkpoint filename prefix')
    parser.add_argument('--no-pseudo', action='store_true',
                        help='Skip pseudo-label generation (real + synthetic only)')
    parser.add_argument('--max-epochs', type=int, default=60,
                        help='Stop epoch (inclusive). 10 for the v12 diagnostics.')
    ...
    if rank == 0:
        print(f"{'═'*60}")
        print(f"  RHAN-v12: Locked-In Active Inference Training")
        print(f"  Device: {device} | DDP: {is_ddp} (world_size={world_size})")
        print(f"  Foraging steps: {args.max_foraging_steps} (FIXED, no halt network)")
        print(f"  Fovea size: {args.fovea_size}")
        print(f"  Gaze lambda (recon-guided): {args.gaze_lambda}")
        print(f"  Loss weights: trades={args.w_trades}, recon={args.w_recon} "
              f"(only two terms — auxiliary losses deleted)")
    ...
    # ── 1. Generate pseudo-labels (unless --no-pseudo) ────────────
    pseudo_indices = None
    pseudo_lbls = None
    ...
    if not args.no_pseudo:
        ...
        if rank == 0:
            ...
            if os.path.exists(best_labeling_ckpt):
                labeling_model.load_state_dict(compat_load(best_labeling_ckpt, map_location=device))
        ...
        print("--no-pseudo active: skipping pseudo-label generation "
              "(real + synthetic only).", flush=True)
    ...
    # Free the raw unlabeled dataset in-memory copy to prevent RAM OOM
    ...
    if os.path.exists(best_target_ckpt):
        ...
        if rank == 0:
            ...
            print(f"  Missing keys (new v12 components): {len(missing)}", flush=True)
    ...
    def set_new_component_training(model, trainable):
        """Toggle training state of new v10/v11/v12 components (no halt_net)."""
        for name, param in model.named_parameters():
            if any(x in name for x in [
                'foveal_stream', 'precision_ctrl', 'action_init',
    ...
    for epoch in range(start_epoch, args.max_epochs + 1):
        ...
        for batch_idx, (imgs, lbls, weights) in enumerate(trainloader):
            ...
            with sync_context:
                ...
                if epoch <= WARMUP_EPOCHS:
                    ...
                    # ── v12 loss: L = w_trades * L_trades + w_recon * L_recon ──
                    with autocast('cuda'):
        ...
        if rank == 0:
            ...
            print(
                f"Epoch {epoch:03d}/{args.max_epochs:03d} (ε={eps:.3f}) | Loss:{total_loss/max(n_total,1):.3f} | "
