#!/usr/bin/env python3
"""
RHAN-v2: Unified Biologically-Grounded Adversarial Training
============================================================
Architecture: RHANSplit (ventral/dorsal)
Base: checkpoints/rhan_adv_best.pth
Loss: 0.4*adv_CE + 0.2*clean_CE + 0.3*align_on_adv + 0.1*consistency
...
def main():
    ...
    output_ckpt = os.path.join(ckpt_dir, 'rhan_v2_best.pth')

    for path, name in [(rhan_ckpt, 'rhan_adv_best.pth'), (cornet_ckpt, 'cornets_best.pth')]:
        if not os.path.exists(path):
            print(f"ERROR: {name} not found at {path}"); return

    # Model — warm-start from RHAN-adv
    model = RHANSplit(head_type='cosine').to(device)
    rhan_state = torch.load(rhan_ckpt, map_location=device)
    filtered = {k: v for k, v in rhan_state.items()
                if k in model.state_dict() and v.shape == model.state_dict()[k].shape}
    missing, _ = model.load_state_dict(filtered, strict=False)
    print(f"Loaded {len(filtered)} layers; new layers: {missing}")
    ...
    epochs = 50; accum_steps = 2
    optimizer = optim.AdamW(model.parameters(), lr=3e-5, weight_decay=0.05)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)
    scaler    = GradScaler('cuda')
    tb_writer = SummaryWriter(log_dir=os.path.join(script_dir, '..', 'runs', 'rhan_v2'))

    W_ADV_CE=0.40; W_CLEAN_CE=0.20; W_ALIGN=0.30; W_CONSIST=0.10

    print("Compiling model via torch.compile...")
    compiled_model = torch.compile(model)

    print(f"\n{'='*70}")
    print("RHAN-v2 · Unified Biologically-Grounded Adversarial Training")
    print(f"{'='*70}")
    print(f"  Architecture:  RHANSplit (ventral/dorsal)")
    print(f"  Base ckpt:     {rhan_ckpt}")
    ...
    print(f"  Alignment:     ON ADVERSARIAL IMAGES (key fix)")
    ...
    for epoch in range(epochs):
        ...
        for step, (imgs, labels) in enumerate(trainloader):
            ...
            with autocast('cuda'):
                ...
                # 3. Adversarial alignment (KEY FIX — align ON adv images)
    ...
    print(f"\n{'='*70}\nRHAN-v2  Inline PGD-100 Evaluation\n{'='*70}")
    for eps in epsilons:
    ...
    print(f"\n{'='*70}\nRHAN-v2  FINAL VERDICT\n{'='*70}")
    print(f"{'ε':<8} | {'RHAN-adv':<12} | {'RHAN-v2':<12} | {'Delta':<10} | Verdict")
    print("-"*65)
    ...
    if v2_005>b_005 and v2_clean>b_clean:
        ...
        print(f"   RHAN-v2 improves BOTH clean ({v2_clean:.2f}% vs {b_clean:.2f}%)")
