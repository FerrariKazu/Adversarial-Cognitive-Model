#!/usr/bin/env python3
"""
RHAN Predictive Coding — Adversarial Fine-Tuning
===================================================
Stage 2: Adversarial fine-tuning with PGD-5 curriculum.
Loads from rhan_predcoding_clean_best.pth.
"""
...
def main():
    ...
    clean_ckpt = os.path.join(ckpt_dir, 'rhan_predcoding_clean_best.pth')

    if not os.path.exists(clean_ckpt):
        print(f"ERROR: Clean checkpoint not found at {clean_ckpt}")
        print("Run train_rhan_predcoding.py first!")
        return
    ...
    scaler = GradScaler()
    tb_writer = SummaryWriter(log_dir=os.path.join(os.path.dirname(__file__), '..', 'runs', 'rhan_predcoding_adv'))

    print("Compiling model via torch.compile...")
    compiled_model = torch.compile(model)

    best_test_acc = 0.0
    output_ckpt = os.path.join(ckpt_dir, 'rhan_predcoding_adv_best.pth')
    ...
    # Epsilon curriculum: ramp up over epochs
    def get_epsilon(epoch, total_epochs):
        progress = epoch / max(total_epochs - 1, 1)
        return 0.031 + (0.10 - 0.031) * progress

    print(f"\n{'='*70}")
    print(f"RHAN Predictive Coding — Adversarial Fine-Tuning")
    print(f"{'='*70}")
    ...
    # Gradient masking check: PGD-20 vs PGD-100 gap
    print(f"\n{'='*70}")
