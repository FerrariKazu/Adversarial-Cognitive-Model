...
def get_dataloaders(batch_size=16):
    # Shape-biased augmentation — grayscale forces shape reliance over color/texture
    train_transforms = transforms.Compose([
        transforms.Resize(64),
        transforms.RandomHorizontalFlip(),
        transforms.RandomGrayscale(p=0.3),   # shape bias: removes color texture cues
...
def main():
    ...
    # Config
    EPOCHS     = 30
    LR         = 0.001   # Low LR — weights are already pretrained
    BATCH_SIZE = 32      # Small batch for 2GB VRAM
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if test_acc > best_acc:
            ...
            torch.save(model.state_dict(),
                       'checkpoints/shaperesnet50_best.pth')
            print(f"  *** New best: {best_acc:.2f}% — checkpoint saved ***")

    print(f"\nTraining complete. Best accuracy: {best_acc:.2f}%")
    print("Checkpoint: checkpoints/shaperesnet50_best.pth")
