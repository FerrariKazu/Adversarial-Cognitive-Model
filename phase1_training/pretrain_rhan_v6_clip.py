#!/usr/bin/env python3
"""
RHAN-v6 Phase 1: CLIP Semantic Initialization
==============================================
Trains the RHANv6 model from random initialization with ONLY clean CLIP alignment
...
Saves to: checkpoints/rhan_v6_clip_init.pth
...
def main():
    ...
    output_ckpt = os.path.join(ckpt_dir, 'rhan_v6_clip_init.pth')

    # ── Model: RHANv6 — randomly initialized ──
    model = RHANv6(head_type='cosine').to(device)
    print("RHANv6 initialized randomly (from scratch).")
    ...
    print(f"\n{'='*70}")
    print("RHAN-v6 Phase 1: CLIP Semantic Initialization")
    print(f"{'='*70}")
    print(f"  Architecture:  RHANv6 (frequency-separated, ventral/dorsal)")
    ...
    for epoch in range(epochs):
        ...
        for imgs, labels in trainloader:
            ...
            with autocast('cuda'):
                # Note: RHANv6 returns logits, features, mean_steps
                logits, features, _ = model.forward_with_features(imgs)
        ...
        with torch.no_grad():
            for inputs, targets in testloader:
                ...
                with autocast('cuda'):
                    # forward returns (logits, mean_steps)
    ...
    print(f"\n{'='*70}")
    print(f"Phase 1 complete. Best test accuracy: {best_test_acc:.2f}%")
