#!/usr/bin/env python3
"""
RHAN-Aligned Fine-Tuning (Trial 8)
==================================
Aligns RHAN's internal representations with primate IT cortex activations 
using a pre-trained CORnet-S model as a teacher. Fits the 512-dim CLS token 
embedding to CORnet-S IT features via a cosine distance loss.
"""
...
from model_rhan_aligned import RHANAligned
...
def get_it_features(teacher_model, x):
    """
    Extracts the 512-dimensional IT activations from a CORnet-S model.
    CORnet-S structure: model -> module -> V1/V2/V4/IT/decoder
    """
    out = teacher_model.model.module.V1(x)
...
def main():
    ...
    # 1. Initialize RHANAligned and load base weights
    ...
    # 2. Initialize CORnet-S teacher and load pre-trained weights
    ...
    # TensorBoard setup
    tb_writer = SummaryWriter(log_dir=os.path.join(os.path.dirname(__file__), '..', 'runs', 'rhan_aligned'))
    ...
    best_test_acc = 0.0
    output_ckpt_path = os.path.join(ckpt_dir, 'rhan_aligned_best.pth')

    print(f"\n{'='*70}")
    print(f"RHAN representation alignment fine-tuning (Trial 8)")
    print(f"{'='*70}")
    print(f"  Base checkpoint:   {rhan_ckpt}")
    print(f"  Teacher model:     CORnet-S (loaded from {cornet_ckpt})")
    print(f"  Optimizer:         AdamW (lr=5e-5, wd=0.05)")
    print(f"  Scheduler:         CosineAnnealingLR (T_max={epochs})")
    print(f"  Batch size:        64")
    print(f"  Epochs:            {epochs}")
    print(f"  Loss weights:      CE = 1.0, Alignment = 0.5")
    ...
    for epoch in range(epochs):
        ...
        for step, (imgs, labels) in enumerate(trainloader):
            ...
            # Get teacher IT features (expects 224x224 inputs, resize clean images only)
            with torch.no_grad():
                imgs_resized = F.interpolate(imgs, size=(224, 224), mode='bilinear', align_corners=False)
                teacher_features = get_it_features(teacher, imgs_resized)
                teacher_features = F.normalize(teacher_features, dim=-1)

            optimizer.zero_grad(set_to_none=True)

            with autocast():
                # Forward pass to get logits and normalized visual features
                logits, features = compiled_model.forward_with_features(mixed_imgs)
                
                # 1. Classification Loss (on clean and adv mix)
                loss_ce = F.cross_entropy(logits, mixed_labels)
                
                # 2. Alignment Loss (Clean features mapped to CORnet IT features)
                # Ensure we align features of clean images (the second half of the batch)
                if half > 0:
                    clean_features_rhan = features[half:]
                    clean_features_teacher = teacher_features[half:]
                else:
                    clean_features_rhan = features
                    clean_features_teacher = teacher_features
                    
                loss_align = 1.0 - (clean_features_rhan * clean_features_teacher).sum(dim=-1).mean()
                
                # Total combined loss
                total_loss = loss_ce + 0.5 * loss_align
            ...
            epoch_align_loss += loss_align.item() * B
        ...
        tb_writer.add_scalar('Loss/CE', epoch_ce_loss, epoch)
        tb_writer.add_scalar('Loss/Alignment', epoch_align_loss, epoch)
        ...
        print(f"Epoch {epoch+1:02d}/{epochs} | CE: {epoch_ce_loss:.4f} | Align: {epoch_align_loss:.4f} | "
