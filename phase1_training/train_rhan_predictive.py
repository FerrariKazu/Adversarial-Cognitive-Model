#!/usr/bin/env python3
"""
RHAN-Predictive Fine-Tuning (Trial 4)
=====================================
Fine-tunes the RHANPredictive architecture (Predictive Coding Feedback)
using 50% clean and 50% PGD-5 (eps=0.031) mixed adversarial training.
"""
...
# Silence TF32 warning and optimize matmul performance
torch.set_float32_matmul_precision('high')

sys.path.insert(0, os.path.dirname(__file__))
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

from model_rhan_predictive import RHANPredictive
...
def main():
    ...
    # 1. Initialize RHANPredictive and load base weights
    ...
    # TensorBoard setup
    tb_writer = SummaryWriter(log_dir=os.path.join(os.path.dirname(__file__), '..', 'runs', 'rhan_predictive'))
    ...
    best_test_acc = 0.0
    output_ckpt_path = os.path.join(ckpt_dir, 'rhan_predictive_best.pth')

    print(f"\n{'='*70}")
    print(f"RHAN Predictive Coding Fine-Tuning (Trial 4)")
    print(f"{'='*70}")
