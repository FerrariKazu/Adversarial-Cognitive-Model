...
from dataset_stl10 import get_stl10_loaders, STL10_CLASSES, STL10_MEAN, STL10_STD
...
def main():
    parser = argparse.ArgumentParser(description='Correct AutoAttack Per-Class Check')
    ...
    # Wrap model for AutoAttack (normalizes unnormalized [0, 1] inputs before inference)
    class CorrectLogitWrapper(nn.Module):
    ...
    wrapper = CorrectLogitWrapper(model, STL10_MEAN, STL10_STD, device)

    # Load data (which is normalized by default)
    ...
    for imgs, lbls in test_loader:
        ...
        if take <= 0:
            break
        all_imgs_norm.append(imgs[:take])
        all_lbls.append(lbls[:take])
        count += take

    x_test_norm = torch.cat(all_imgs_norm, dim=0).to(device)
    y_test = torch.cat(all_lbls, dim=0).to(device)
    N = x_test_norm.size(0)
    print(f"Loaded {N} samples", flush=True)

    # Unnormalize images to [0, 1] for AutoAttack compatibility
    print("Unnormalizing images to [0, 1] range for AutoAttack compatibility...", flush=True)
    ...
    with torch.no_grad():
        for i in range(0, N, args.batch_size):
            x_b = x_test_unnorm[i:i+args.batch_size]
    ...
    print(f"\nRunning AutoAttack on [0, 1] range (eps={args.eps}, version=standard)...", flush=True)
    from autoattack import AutoAttack
    adversary = AutoAttack(wrapper, norm='Linf', eps=args.eps, version='standard', device=device, verbose=True)
    
    # Run AA standard evaluation (it will perturb and clamp strictly within [0, 1])
    x_adv_unnorm = adversary.run_standard_evaluation(x_test_unnorm, y_test, bs=args.batch_size)
    
    # Evaluate robust predictions
    print("\nEvaluating AutoAttack robust predictions...", flush=True)
    with torch.no_grad():
        for i in range(0, N, args.batch_size):
            x_b_adv = x_adv_unnorm[i:i+args.batch_size]
            y_b = y_test[i:i+args.batch_size]
    ...
    print("\n" + "="*80)
    print(f" CORRECT AUTOATTACK PER-CLASS ACCURACY REPORT (n={N}, eps={args.eps})")
    print("="*80)
