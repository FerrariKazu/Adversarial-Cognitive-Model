#!/usr/bin/env python3
"""
Predictive Coding RHAN vs. Epsilon Analysis (Trial 4 Evaluation)
================================================================

Evaluates the trained Predictive Coding RHAN model under PGD-100 attacks 
at multiple epsilon levels and compares its performance with RHAN-adv.
Saves diagnostic plot to:
  phase4_analysis/figures/combined/predictive_accuracy_vs_epsilon.png
"""
...
from phase1_training.model_rhan_predictive import RHANPredictive
...
def main():
    ...
    predictive_ckpt = os.path.join(ckpt_dir, 'rhan_predictive_best.pth')

    if not os.path.exists(predictive_ckpt):
        print(f"ERROR: Predictive RHAN checkpoint not found at {predictive_ckpt}")
        print("Please run train_rhan_predictive.py first!")
        return

    # 1. Load model (Eager mode)
    print("Loading Predictive RHAN model...")
    model = RHANPredictive(head_type='cosine').to(device)
    model.load_state_dict(torch.load(predictive_ckpt, map_location=device))
    model.eval()
    ...
    accuracies = []
    print(f"\n{'='*70}")
    print(f"Predictive Coding RHAN Evaluation (PGD-100)")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print(f"TRIAL 4 FINAL VERDICT")
    print(f"{'='*70}")
    print(f"{'ε':<8} | {'RHAN-adv':<12} | {'RHAN-predictive':<18} | {'Delta':<10}")
    print("-" * 55)
    for i, eps in enumerate(epsilons):
        adv_acc = rhan_adv_pgd[eps]
        predictive_acc = accuracies[i]
        delta = predictive_acc - adv_acc
        sign = "+" if delta >= 0 else ""
        print(f"{eps:<8.2f} | {adv_acc:<11.2f}% | {predictive_acc:<17.2f}% | {sign}{delta:<8.2f}%")
    print("=" * 55)

    # PLOT
    os.makedirs('phase4_analysis/figures/combined', exist_ok=True)
    fig_path = 'phase4_analysis/figures/combined/predictive_accuracy_vs_epsilon.png'
    
    plt.figure(figsize=(7, 5))
    plt.plot(epsilons, [rhan_adv_pgd[e] for e in epsilons], '-o', label='RHAN-adv (Standard Feedback)', color='#00ff7f', linewidth=2)
    plt.plot(epsilons, accuracies, '-s', label='RHAN-Predictive (Predictive Coding)', color='#1e90ff', linewidth=2)
    plt.title("Adversarial Robustness: Standard vs. Predictive Feedback", fontsize=12, fontweight='bold')
